...
A surface in 3D is also the $0$-level set of an implicitly defined function in a $4$-dimensional space, where one of the four dimensions can be isolated (see Implicit Function Theorem). \todo[inline]{MG: There is confusion here: $F(\mathbf{x},y)$ is not a manifold, it is just a scalar function of vectorial argument. The manifold is $F(\mathbf{x},y)=0$ and this defines, under the Implicit Function theorem hypotheses, locally an explicit function $y=f(\mathbf{x})$: please use a different symbol as $F$ and $f$ are different things. Then, since the roles of $y$ (parameter) and $\mathbf{x}$ are very different you cannot say there is a 4D space: the space is 3D and you have a $2D$ family of surfaces (like the sphere not a ball!) in $3D$ as $y$ varies. Imagine a family of spheres of different radii $y$: 2D objects (a surface) in 3D.}
